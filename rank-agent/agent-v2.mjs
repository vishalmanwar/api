const AGENT_VERSION = "2026.09.23.2";
...
async function loadSearch(page,keyword,pageNum=1){
  ...
  for(let a=1;a<=4;a++){
    try{
      ...
      console.log('Amazon attempt',a,'status',lastStatus,'title',last,'bodyChars',body.length,'cards',cards);
      if(body.trim().length>200 && cards>0 && !/503 - Service Unavailable|Robot Check|Enter the characters/i.test(body)) return;
    }catch(e){
      lastError=e?.message||String(e);
      console.log('Amazon attempt',a,'error',lastError);
    }
  ...
  throw new Error('Amazon search unavailable. status='+lastStatus+' title='+last+' url='+finalUrl+' bodyChars='+body.length+' lastError='+lastError+' screenshot='+shot);
}
