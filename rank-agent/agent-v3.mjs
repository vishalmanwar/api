...
console.log("Zipify Rank Agent version:", AGENT_VERSION);
...
async function ensureAmazonHome(page){
  const bodyNow=await page.locator('body').innerText().catch(()=>'');
  const onAmazon=page.url().startsWith('https://www.amazon.in/') && bodyNow.trim().length>200;
  if(onAmazon) return;

  let lastStatus=null,lastTitle='',lastError='';
  for(let a=1;a<=4;a++){
    try{
      const resp=await page.goto('https://www.amazon.in/',{waitUntil:'domcontentloaded',timeout:60000});
      lastStatus=resp?.status()??null;
      await page.waitForTimeout(1800+a*700);
      lastTitle=await page.title().catch(()=>'');
      const body=await page.locator('body').innerText().catch(()=>'');
      console.log('Amazon home attempt',a,'status',lastStatus,'title',lastTitle,'bodyChars',body.length);
      if(body.trim().length>200 && !/503 - Service Unavailable|Robot Check|Enter the characters/i.test(body)) return;
    }catch(e){
      lastError=e?.message||String(e);
      console.log('Amazon home attempt',a,'error',lastError);
    }
    await page.waitForTimeout(800*a);
  }
  const shot=path.join(here,'amazon-home-failure-'+Date.now()+'.png');
  await page.screenshot({path:shot,fullPage:false}).catch(()=>{});
  throw new Error('Amazon homepage unavailable. status='+lastStatus+' title='+lastTitle+' lastError='+lastError+' screenshot='+shot);
}

async function submitSearch(page,keyword){
  await ensureAmazonHome(page);
...
async function loadSearch(page,keyword,pageNum=1){
  let lastTitle='',lastError='',lastStatus=null;
  for(let a=1;a<=4;a++){
    try{
      if(pageNum===1){
        await submitSearch(page,keyword);
      } else {
        // Use Amazon's visible pagination control instead of direct deep-link navigation.
        const next=page.locator('a.s-pagination-next:not(.s-pagination-disabled)');
        if(await next.count()){
          await Promise.all([
            page.waitForLoadState('domcontentloaded',{timeout:60000}).catch(()=>{}),
            next.first().click()
          ...
          throw new Error('Next page control not available');
        }
      }
      ...
      console.log('Amazon search attempt',a,'page',pageNum,'title',lastTitle,'bodyChars',body.length,'cards',cards,'url',page.url());
      if(body.trim().length>200 && cards>0 && !/503 - Service Unavailable|Robot Check|Enter the characters/i.test(body)) return;
    }catch(e){
      lastError=e?.message||String(e);
      console.log('Amazon search attempt',a,'page',pageNum,'error',lastError);
    }
    // Reset to home before retrying page 1.
    if(pageNum===1){
      await page.goto('https://www.amazon.in/',{waitUntil:'domcontentloaded',timeout:60000}).catch(()=>{});
  ...
  await page.screenshot({path:shot,fullPage:false}).catch(()=>{});
  throw new Error('Amazon search unavailable. title='+lastTitle+' url='+page.url()+' bodyChars='+body.length+' lastError='+lastError+' screenshot='+shot);
}
